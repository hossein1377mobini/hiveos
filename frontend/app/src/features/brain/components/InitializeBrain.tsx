...
// Mockup 05: the brain returns its IDs + model configuration after initialization.
// Per terminology.md §7 (PO, 2026-08-18), technical retrieval/embedding/index
// terms must NOT appear in the user-facing UI — Persian equivalents are used.
const FIELDS: { key: keyof BrainInitResult | "rag.embeddingProvider" | "rag.defaultLanguage"; label: string; mono?: boolean }[] = [
  ...
  { key: "vectorIndexId", label: "شناسه جست‌وجوی دانش", mono: true },
  { key: "rag.embeddingProvider", label: "ارائه‌دهنده مدل" },
...
export default function InitializeBrain({ onDone, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await brainApi.initializeBrain();
