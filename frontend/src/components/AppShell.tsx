...
const NAV_ITEMS: ReadonlyArray<{ label: string; icon: ComponentType<{ className?: string }> }> = [
  { label: "گفتگو", icon: MessageCircle },
  { label: "دانش سازمان", icon: BookOpen },
  { label: "اعتبار و مصرف", icon: CreditCard },
  { label: "کیف پول", icon: Wallet },
  { label: "اشتراک", icon: CalendarClock },
];
        ...
        <nav aria-label="ناوبری اصلی" className="w-56 shrink-0 border-e border-neutral-200 bg-neutral-0 p-3">
          <ul className="space-y-1">
            {NAV_ITEMS.map(({ label, icon: Icon }) => (
              <li key={label}>
                <span className="flex items-center gap-2 rounded-control px-3 py-2 text-sm text-neutral-600">
